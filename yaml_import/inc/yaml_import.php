...
// Version 0.3:  Adaptation to 0.20.x, added v6 support, detect virtual or real
        ...
        if ($handle = opendir('../yamls/'))
        {
            while (false !== ($file = readdir($handle)))
            {
                ...
                if($file == $objectname . ".yaml")
                {
                    ...
                    $yaml_file_array = $yaml_file_array['values'];
                    ...
                    if($object)
                    {
                        ...
                        if($is_virtual != "true")
                        {
                            ...
                            if(isset($yaml_file_array['hypervisor']))
                            {
                                ...
                                commitUpdateAttrValue ($object_id = $id, $attr_id = '26', $value = $hv_dict_key);
                            }
                        ...
                        //commitUpdateAttrValue ($object_id = $id, $attr_id = '10000', $value = $yaml_file_array['hardwareisa']);
                        // Memory. Attribute ID is 17.
                        $mem_si = $yaml_file_array['memorysize'];
                        $mem_size = preg_replace("/GB/","", $mem_si)*1024;

                        commitUpdateAttrValue ($object_id = $id, $attr_id = '17', $value = $mem_size);

                        // CPU. Attribute ID is 100001
                        $cpu_count = $yaml_file_array['processorcount'];
                        $cpu_sp = end(array_map('trim', explode(' ', $yaml_file_array['processor0'])));
                        $cpu_speed = preg_replace("/GHz/","", $cpu_sp)*1000;

                        commitUpdateAttrValue ($object_id = $id, $attr_id = '10001', $value = $cpu_count);
                        commitUpdateAttrValue ($object_id = $id, $attr_id = '18', $value = $cpu_speed);
                        $f = fopen("/tmp/test.txt", "w"); 
                        fwrite($f,"$cpu_sp $cpu_speed"); 
                        fclose($f);  
                        // OEM S/N 1. Attribute ID is '1'.
                        commitUpdateAttrValue ($object_id = $id, $attr_id = '1', $value = $yaml_file_array['serialnumber']);
                        // FQDN. Attribute ID is '3'.
                        commitUpdateAttrValue ($object_id = $id, $attr_id = '3', $value = $yaml_file_array['fqdn']);
                        // UUID. Attribute ID is '25'.
                        commitUpdateAttrValue ($object_id = $id, $attr_id = '25', $value = $yaml_file_array['uuid']);

                        // Puppet - machine is running puppet so set attribute 10002
                        commitUpdateAttrValue ($object_id = $id, $attr_id = '10002', $value = 1501);
...
//
       ...
       //                         commitUpdatePort($object_id = $id, $nics[$i], 40,'infiniband',$mac);
      //                          // Add IP to object.
     //                           if(isset($ip))
    //                            {
   //                                 bindIpToObject($ip , $id, $nics[$i],'regular');
  //                              }
 //                               break 1;
//
                          //  default:
                     ...
                     //               $ip = $yaml_file_array['ipaddress_' . $nics[$i]];
                    //            }
                   //             if (isset($yaml_file_array['ipaddress6_' . $nics[$i]]))
                  //              {
                 //                   $ip6 = $yaml_file_array['ipaddress6_' . $nics[$i]];
                //                }
               //                 if (isset($yaml_file_array['macaddress_' . $nics[$i]]))
              //                  {
             //                       $mac = $yaml_file_array['macaddress_' . $nics[$i]];
          ...
          //                      commitUpdatePort($object_id = $id, $nics[$i], 24,'Ethernet port',"$mac");
         //                       // Add IP to object.
        //                        if(isset($ip))
       //                         {
      //                              bindIpToObject(ip4_parse($ip) , $id, $nics[$i],'regular');
     //                           }
    //                            if(isset($ip6))
  //                              {
   //                                 bindIpToObject(ip6_parse($ip6) , $id, $nics[$i],'regular');
//                                }
 //                               break 1;
//
                       //     }
                       ...
                       // }
                        ...
                        $log = "<a href=\"$url\">" . $objectname .  "</a> updated!";
                    ...
                    else
                    {
                        ...
                        if($is_virtual == "true")
                        {
                            $new_yamlobject = commitAddObject ($yaml_name,'',1504,$yaml_file_array['serialnumber'],$taglist);
                        }
                        else
                        {
                        ...
                        commitUpdateAttrValue ($object_id = $new_yamlobject, $attr_id = '17', $value = $mem_size);
                        ...
                        commitUpdateAttrValue ($object_id = $new_yamlobject, $attr_id = '10001', $value = $cpu_count);
                        commitUpdateAttrValue ($object_id = $new_yamlobject, $attr_id = '18', $value = $cpu_speed);
                        $f = fopen("/tmp/test.txt", "w"); 
                        fwrite($f,"$cpu_sp $cpu_speed"); 
                        fclose($f);  
                        // OEM S/N 1. Attribute ID is '1'.
                        ...
                        // Puppet - machine is running puppet so set attribute 10002
                        commitUpdateAttrValue ($object_id = $new_yamlobject, $attr_id = '10002', $value = 1501);
                        ...
                        for ($i = 0; $i < $count; $i++)
                        {
                            ...
                            default:
                                ...
                                if(isset($ip))
                                {
                                    bindIpToObject(ip4_parse($ip) , $new_yamlobject, $nics[$i],'regular');
                                }
                                if(isset($ip6))
                                {
                                    bindIpToObject(ip6_parse($ip6) , $new_yamlobject, $nics[$i],'regular');
                                }
                                break 1;
                        ...
                        $log = "<a href=\"$url\">" . $objectname .  "</a> created!";
                    }
                }
            }
        }
    ...
    if ($files = scandir('../yamls/'))
    {
        ...
        foreach($files as $file)
        {
            ...
            if(preg_match('/\.yaml/',$file))
            {
                ...
                $yaml_file_array = Spyc::YAMLLoadString($file_contents);
                $yaml_expiry_date = explode(" ", $yaml_file_array['expiration']);

        		$todays_date = date("Y-m-d");

		        $last_week = strtotime ( '-1 week' , strtotime($todays_date));
	        	$expiration_date = strtotime($yaml_expiry_date[0]);

	        	if ($expiration_date < $last_week) {
		            continue;
		        }
